...
using MODEL = CodeBrix.Graphics3D.Gltf2.Schema2.ModelRoot;
using VALIDATIONMODE = CodeBrix.Graphics3D.Gltf2.Validation.ValidationMode;
...
public class ReadContext : ReadSettings
{
    ...
    public static ReadContext CreateFromDirectory(DirectoryInfo dinfo)
    {
        ...
        string _uriSolver(string rawUri)
        {
            ...
            // Some exporters write Windows '\' separators into relative URIs. Windows
            // resolves them either way, but Linux and macOS treat '\' as part of the
            // file name, so normalise to '/' for the same result on every platform.
            path = path.Replace('\\', '/');
    ...
    private static MODEL _FilterErrors((MODEL Model, Validation.ValidationResult Validation) mv)
    {
        if (mv.Validation.HasErrors)
        {
            ...
            CodeBrix.Graphics3D.Gltf2.Validation.ModelException._Decorate(ex);
            throw ex;
        }
