...
public class AffineTransformMatrixTests
{
    ...
    [InlineData(-1, 1, 3, 2, 0, 1)]
    [InlineData(0, 0, 1, 0, 1, 0)]
    [InlineData(0, -1, 1, -2, 1, 0)]
    ...
    public void TestMatrixNormalization()
    {
        void testMatrix(Matrix4x4 m, float tolerance = 0)
        {
            ...
            Matrix4x4.Decompose(m, out _, out _, out _).Should().BeTrue();
            Matrix4x4.Invert(m, out _).Should().BeTrue();
        }
        ...
        void testSkewed(Func<Matrix4x4, Matrix4x4> mf, float tolerance = 0)
        {
            ...
            Matrix4x4.Decompose(m, out _, out _, out _).Should().BeFalse();
    ...
    public void TestAffineTransformIdentity()
    {
        ...
        asMatrix.IsIdentity.Should().BeTrue();
        asDecomposed.IsIdentity.Should().BeTrue();
    }
    ...
    public void TestAffineTransformMult(bool sa, bool sb, bool ra, bool rb)
    {
        ...
        TestContext.Current.TestOutputHelper?.WriteLine($"A({sa},{ra}) x B({sb},{rb}) = {srt_ab.IsSRT}");
        TestContext.Current.TestOutputHelper?.WriteLine($"B({sb},{rb}) x A({sa},{ra}) = {srt_ba.IsSRT}");
    ...
    [Theory]
    [InlineData(true, 1, 10, 100, 0, 0, 0, 5, 5, 5)]
    [InlineData(true, 1, 1, 1, 0, 0, 0, 0, 0, 0)]
    [InlineData(true, 1, 1, 1, 2, 0, -1, 100, 50, 0)]
    [InlineData(true, 5, 5, 5, 1, 2, 3, 100, 50, 0)]
    [InlineData(false, 1, 2, 3, 1, 2, 3, 1, 2, 3)]
    [InlineData(false, 1, 2, 30, 1, 2, 3, 1, 2, 3)]
    [InlineData(false, -1, -2, 3, 0, 1, 0, 1, 0, 0)]
    public void TestAffineTransformInverse(bool isInvertibleToSRT, float sx, float sy, float sz, float y, float p, float r, float tx, float ty, float tz)
    {
        ...
        AffineTransform.TryInvert(xf, out var xi).Should().BeTrue();
        Matrix4x4.Invert(xf.Matrix, out var mi).Should().BeTrue();
        mi.M44 = 1f;

        if (isInvertibleToSRT) xi.IsSRT.Should().BeTrue();
        ...
        TestContext.Current.TestOutputHelper?.WriteLine($"{diff}");

        AffineTransform.AreGeometricallyEquivalent(mi, xi, tolerance).Should().BeTrue();
    }
}
