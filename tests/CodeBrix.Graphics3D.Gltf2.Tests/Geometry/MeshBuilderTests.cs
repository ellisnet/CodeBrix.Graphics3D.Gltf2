...
using Xunit;
using VERTEX1 = CodeBrix.Graphics3D.Gltf2.Geometry.VertexBuilder<CodeBrix.Graphics3D.Gltf2.Geometry.VertexTypes.VertexPosition, CodeBrix.Graphics3D.Gltf2.Geometry.VertexTypes.VertexColor1Texture1, CodeBrix.Graphics3D.Gltf2.Geometry.VertexTypes.VertexJoints4>;
using VERTEX2 = CodeBrix.Graphics3D.Gltf2.Geometry.VertexBuilder<CodeBrix.Graphics3D.Gltf2.Geometry.VertexTypes.VertexPositionNormal, CodeBrix.Graphics3D.Gltf2.Geometry.VertexTypes.VertexColor1Texture1, CodeBrix.Graphics3D.Gltf2.Geometry.VertexTypes.VertexJoints4>;
...
public class MeshBuilderTests
{
    ...
    public void CreateInvalidTriangles()
    {
        ...
        Assert.Throws<ArgumentException>(() => prim.AddTriangle(a.WithGeometry(v3nan), b, c));
        TriangleCounter().Should().Be(1);

        Assert.Throws<ArgumentException>(() => prim.AddTriangle(a.WithGeometry(Vector3.Zero, v3nan), b, c));
        TriangleCounter().Should().Be(1);

        Assert.Throws<ArgumentOutOfRangeException>(() => prim.AddTriangle(a.WithGeometry(Vector3.Zero, Vector3.Zero), b, c));
        TriangleCounter().Should().Be(1);

        Assert.Throws<ArgumentOutOfRangeException>(() => prim.AddTriangle(a.WithGeometry(Vector3.Zero, Vector3.UnitX * 0.8f), b, c));
        TriangleCounter().Should().Be(1);

        Assert.Throws<ArgumentException>(() => prim.AddTriangle(a.WithMaterial(v2nan), b, c));
        TriangleCounter().Should().Be(1);

        Assert.Throws<ArgumentException>(() => prim.AddTriangle(a.WithMaterial(v4nan), b, c));
        TriangleCounter().Should().Be(1);

        Assert.Throws<ArgumentOutOfRangeException>(() => prim.AddTriangle(a.WithMaterial(Vector4.One*2), b, c));
        TriangleCounter().Should().Be(1);

        Assert.Throws<ArgumentOutOfRangeException>(() => prim.AddTriangle(a.WithMaterial(-Vector4.One), b, c));
        TriangleCounter().Should().Be(1);

        Assert.Throws<ArgumentOutOfRangeException>(() => prim.AddTriangle(a.WithSkinning((0,0)), b, c));
        TriangleCounter().Should().Be(1);
    }
    ...
    public static void CreateWithDegeneratedTriangle()
    {
        ...
        validIndices.A.Should().BeGreaterThanOrEqualTo(0);
        validIndices.B.Should().BeGreaterThanOrEqualTo(0);
        validIndices.C.Should().BeGreaterThanOrEqualTo(0);
        ...
        degenIndices.A.Should().BeLessThan(0);
        degenIndices.B.Should().BeLessThan(0);
        degenIndices.C.Should().BeLessThan(0);
        ...
        model.LogicalMeshes[0].Primitives.Should().HaveCount(1);
    }
    ...
    public static void CreateWithMutableSharedMaterial()
    {
        ...
        Materials.MaterialBuilder.AreEqualByContent(material1, material2).Should().BeTrue();
        Materials.MaterialBuilder.AreEqualByContent(material1, material3).Should().BeTrue();

        (material2 != material1).Should().BeTrue();
        (material3 != material1).Should().BeTrue();
        ...
        mesh.Primitives.Should().HaveCount(3);
        ...
        gltfModel.LogicalMaterials.Should().HaveCount(1);
        ...
        gltfModel.LogicalMaterials.Should().HaveCount(3);
    ...
    public void CreateMeshWithTriangleAndQuad()
    {
        ...
        prim.Vertices.Should().HaveCount(7);
        prim.Triangles.Should().HaveCount(3);
        prim.Surfaces.Should().HaveCount(2);

        triIdx.Should().Be((0, 1, 2));
        qadIdx.Should().Be((3, 4, 5, 6));

        prim.GetIndices().Should().Equal(new[] { 0, 1, 2, 3, 4, 5, 3, 5, 6 });
    }
    ...
    public void CreateMeshWithCustomVertexAttribute()
    {
        ...
        batchId.Should().Equal(new float[] { 0.1f, 0.2f, 0.3f });
    }
    ...
    public void CloneMeshBuilder()
    {
        ...
        {
            dst.Material.Should().NotBeSameAs(src.Material);
            dst.Triangles.Should().HaveCount(src.Triangles.Count);

            dst.Triangles.Should().Equal(src.Triangles);
        }
        ...
        cloned2.Primitives.Count.Should().Be(1);
        cloned2.Primitives.First().Material.Should().BeSameAs(material3);
        cloned2.Primitives.Sum(item => item.Triangles.Count).Should().Be(cloned1.Primitives.Sum(item => item.Triangles.Count));
    }
}
