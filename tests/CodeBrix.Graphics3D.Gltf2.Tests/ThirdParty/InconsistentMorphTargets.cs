...
public class InconsistentMorphTargets
{
    ...
    public void BuildMesh_WithPartiallyAppliedMorphTargets_AllPrimitivesShouldHaveSameTargetCount(bool use_temp_fix)
    {
        ...
        model.LogicalMeshes.Count.Should().Be(1, "The model should have only one mesh");
        var logicalMesh = model.LogicalMeshes[0];

        logicalMesh.Primitives.Count.Should().Be(3, "The mesh should have three primitives.");
        ...
        prim1_test.MorphTargetsCount.Should().Be(expectedTargetCount, "The number of morph targets of the second primitive is not equal to the first one.");
        prim2_test.MorphTargetsCount.Should().Be(expectedTargetCount, "The number of morph targets of the third primitive is not equal to the first one.");
    }
}
